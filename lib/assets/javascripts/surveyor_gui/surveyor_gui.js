...
//      if (!Modernizr.input.required) {
    ...
    $('body').on('click','#edit_section_title, #add_section, #add_question, #add_logic', function () {
    ...
    $(document).on('click', '#delete_section', function() {
        ...
        var result = confirm("Are you sure you want to delete section " + survey_section_title + "?")
    ...
    $(document).on('click', '#paste_question', function() {
        ...
        //provides a way to check that this function is complete in tests.  It disappears on complete when
        //the contents of div#box are replaced.
        $('div#survey').addClass('jquery_paste_question_started');
...
function show_dependency_value(data, target) {
    target.closest('.question-row').find('.answer_field');
    data = data.split(',');
...
function remove_dependency_condition(link) {
    ...
    if ($('.question-row:visible').length == 1) { $('.question-row:visible select[id$="join_operator"]').parents('.row').hide(); }
...
$.fn.refresh_dependency_answers = function() {
    var question_id =
            $(this).closest(".question-row").find('[id*="dependency_attributes_dependency_conditions_attributes"][id$="question_id"]').val();
    var column_id   =
            $(this).closest(".question-row").find('[id*="dependency_attributes_dependency_conditions_attributes"][id$="column_id"]').val();
    var dependency_condition_id =
            $(this).closest(".question-row").find('[id*="dependency_attributes_dependency_conditions_attributes"][id$="_id"]').val();
    $(this).closest('.question-row').find('[id*="dependency_attributes_dependency_conditions_attributes"][id$="answer_id"]')
        .load(
...
function initTinyMce() {
    tinymce.init({
        selector: ".tinymce",
        theme: 'modern',
        plugins: "preview,lists,charmap,link",
        toolbar: "undo | redo | bold | italic | underline | alignleft | aligncenter | alignright | alignjustify | styleselect | formatselect | fontselect | fontsizeselect | bullist | numlist | outdent | indent | link | preview ",
